...
class EdsManager:
    ...
    def _restart_ncalayer(cls) -> None:
        ...
        try:
            ...
            cache_dir = os.path.expanduser("/home/asus/.config/NCALayer/ncalayer-cache")
            if os.path.exists(cache_dir):
                shutil.rmtree(cache_dir)
                logger.info(f"NCAlayer сброшен.")
            # Скопировать плагин
            source_file = os.path.expanduser("/ncalayer/kz.ecc.NurSignBundle_5.1.1_2e62beae-e900-4c8c-9d8e-37286ace46ec.jar")
            dest_dir = os.path.expanduser("/home/asus/.config/NCALayer/bundles")
            ...
            env = os.environ.copy()
            env['DISPLAY'] = ':99'
            a = subprocess.run(['sh', script_path, "--restart"], check=True, env=env)
